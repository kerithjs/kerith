...
export async function runFixture(
  ...
  const { healthTimeoutMs = 10_000, env = {}, debug = false, routePrefix = '' } = opts;

  const serverEntry = resolveServerEntry(fixtureDir);
  ...
  await waitForHealth(port, healthTimeoutMs, routePrefix);
...
/**
 * Resolves the server entry point — prefers `src/server.ts` (TypeScript project)
 * but falls back to `src/server.js` for JavaScript projects generated by create-kerith.
 */
function resolveServerEntry(fixtureDir: string): string {
  const ts = resolve(fixtureDir, 'src/server.ts');
  if (existsSync(ts)) return ts;
  return resolve(fixtureDir, 'src/server.js');
}
