...
const runInTmpApp = async (files: Record<string, string>, tests: (tmpDir: string, app: any) => Promise<void>) => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kerith-app-step0-test-'));
...
describe('createApp - Step 0 (Pre-loader Validation)', () => {
  ...
  it('createApp() returns runtime.preloaderActive: true when global is present', async () => {
    (globalThis as any).__KERITH_PRELOAD_CONFIG__ = { preloaded: true, aliases: { '@test': 'test' } };
    
    await runInTmpApp(validAppStructure, async (_, app) => {
      const KerithApp = await createApp(app as any);
      expect(KerithApp.runtime.preloaderActive).toBe(true);
      expect(KerithApp.runtime.aliasesAtBoot).toEqual(expect.objectContaining({ '@test': 'test' }));
    });
  });
  ...
  it('createApp() returns runtime.preloaderActive: false when global is not present', async () => {
    await runInTmpApp(validAppStructure, async (_, app) => {
      const KerithApp = await createApp(app as any);
      expect(KerithApp.runtime.preloaderActive).toBe(false);
    });
  });
  ...
  it('createApp() throws PRELOADER_REQUIRED if requirePreloader is true and pre-loader is not active', async () => {
    await runInTmpApp(validAppStructure, async (tmpDir, app) => {
      fs.writeFileSync(path.join(tmpDir, 'kerith.config.js'), 'export default { requirePreloader: true };');
      await expect(createApp(app as any))
        .rejects.toThrow('The application requires the Kerith pre-loader to be active');
    });
  });
  ...
  it('createApp() emits log.warn of version mismatch when _version does not match', async () => {
    const logger = vi.fn();
    (globalThis as any).__KERITH_PRELOAD_CONFIG__ = { preloaded: true, _version: '0.0.0-mismatch', aliases: {} };
  ...
  it('createApp() emits version mismatch warning when pre-loader was generated with an older version (1.4.0)', async () => {
    const logger = vi.fn();
    // Simulate a pre-loader generated by kerith-core v1.4.0 while
    ...
    await runInTmpApp(validAppStructure, async (_, app) => {
      ...
      expect(logger).toHaveBeenCalledWith(
        'warn',
        expect.stringContaining('kerith sync-preload'),
        expect.any(Object)
      );
    });
  });
