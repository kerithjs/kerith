...
describe('Preload API (index.ts)', () => {
  ...
  it('isPreloaderActive() returns true when preloaded is true', () => {
    (globalThis as any).__KERITH_PRELOAD_CONFIG__ = { preloaded: true };
    expect(isPreloaderActive()).toBe(true);
  });

  it('isPreloaderActive() returns false when __KERITH_PRELOAD_CONFIG__ is undefined', () => {
    expect(isPreloaderActive()).toBe(false);
  });

  it('isPreloaderActive() returns false when __KERITH_PRELOAD_CONFIG__ is defined but preloaded is not true', () => {
    (globalThis as any).__KERITH_PRELOAD_CONFIG__ = { someOtherField: true };
    expect(isPreloaderActive()).toBe(false);
  });

  it('getPreloadConfig() returns the object', () => {
    const config = { preloaded: true, _version: '1.5.0' };
    (globalThis as any).__KERITH_PRELOAD_CONFIG__ = config;
