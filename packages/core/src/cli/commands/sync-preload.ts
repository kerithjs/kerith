...
export async function runSyncPreload(logger: any, silent: boolean = false) {
    ...
    const hasJsConfig = fs.existsSync(path.join(cwd, 'kerith.config.js'));
    const hasTsConfig = fs.existsSync(path.join(cwd, 'kerith.config.ts'));
...
export function syncPreloadCommand(): Command {
  ...
  sync
    .description('Generates .kerith/preload.js embedding runtime aliases configuration')
